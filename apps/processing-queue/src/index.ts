...
async function createSandbox(apiKey: string): Promise<{ daytona: Daytona; sandbox: Sandbox }> {
  ...
  const sandbox = await daytona.create({ snapshot: "glimpse-v6" });
...
// Script + cursor PNG are baked into the Daytona snapshot at /opt/aura/
// Only the events JSON (dynamic per-run) needs to be written.
...
async function postprocessVideo(
  ...
  // Write events JSON — the only dynamic input
  const eventsB64 = Buffer.from(JSON.stringify(events)).toString("base64");
  await sandbox.process.executeCommand(
    `echo '${eventsB64}' | base64 -d > /tmp/aura_events.json`,
  );

  // Run the pre-baked postprocess script from the snapshot
  const result = await sandbox.process.executeCommand(
    `node /opt/aura/postprocess.mjs "${videoPath}" "${outputPath}" /tmp/aura_events.json /opt/aura/cursor.png ${width} ${height}`,
